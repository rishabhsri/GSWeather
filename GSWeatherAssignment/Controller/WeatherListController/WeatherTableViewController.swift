...
class WeatherTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        return weatherListViewModel?.weatherDataList.count ?? 0
    }

    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: weatherCellReuseIdentifier, for: indexPath)
        if let weatherCell = cell as? WeatherTableViewCell, let weatherArray = weatherListViewModel?.weatherDataList {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if let weatherArray = weatherListViewModel?.weatherDataList {
